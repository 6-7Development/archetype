...
async function commitFile() {
  ...
  const { data: commit } = await octokit.git.createCommit({
    owner, repo,
    message: 'fix(railway): Skip drizzle interactive prompts on deployment\n\nDrizzle-kit push causes interactive prompts about table renames\nwhich blocks Railway deployment. Skip drizzle on Railway since\ntables are created via SQL files.',
    tree: tree.sha,
