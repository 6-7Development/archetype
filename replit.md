...
## Recent Changes
- **3-Tier Intelligent Routing for Self-Healing (2024-11-03):** Implemented cost-optimized incident resolution with intelligent AI routing: **Tier 1 - Knowledge Base Auto-Fix** (0 tokens, ≥90% confidence) applies proven fixes instantly; **Tier 2 - LomuAI/Gemini** (cheap, for platform failures) uses Gemini 2.5 Flash; **Tier 3 - I AM Architect/Claude** (expensive, for agent failures) uses Claude Sonnet 4. AgentFailureDetector classifies incidents as platform_failure vs agent_failure to route appropriately. Session metadata tracks aiStrategy, knowledgeBaseMatched, knowledgeMatchConfidence for feedback loops. Massive token savings: KB matches = $0, platform issues = 97% cheaper, complex agent issues = expert review only when needed.
- **Knowledge Management & Platform Evolution Tools (2024-11-03):** Added 4 critical tools for platform learning and evolution: knowledge_store, knowledge_search, knowledge_recall, and code_search. LomuAI can now store learned patterns/fixes/decisions, search shared knowledge base, recall specific solutions, and manage reusable code snippets. These tools enable platform evolution tracking and knowledge accumulation across all AI sessions. Includes comprehensive knowledge workflow instructions for when to store/search knowledge.
- **Complete Tool Inventory (2024-11-03):** System prompt now documents ALL 38 available tools including previously missing project file operations (readProjectFile, writeProjectFile, createProjectFile, deleteProjectFile, listProjectDirectory), platform tools (createPlatformFile, deletePlatformFile), and diagnostic tools (perform_diagnosis, read_logs). LomuAI now has complete visibility into all capabilities.
- **TRUE Replit Agent Behavioral Parity (2024-11-03):** Complete system prompt rewrite in lomuSuperCore.ts to match Replit Agent's autonomous, verbose, and proactive behavior. Replaced "calm_minimalist" token-obsessed mode with professional autonomous engineer style. Key improvements: (1) Mandatory task decomposition for multi-step work, (2) Verbose proactive communication explaining plans and progress, (3) Plan/Execute/Validate/Verify/Confirm workflow, (4) Self-correction with retry logic, (5) Comprehensive tool usage guidelines for all tools, (6) Architect review integration for substantial changes. LomuAI now behaves identically to Replit Agent with same autonomy, communication style, and tool usage patterns.
...
- **Platform Healing System**: Complete self-healing infrastructure with 3-tier intelligent routing for cost-optimized incident resolution: (1) Knowledge Base Auto-Fix (0 tokens, ≥90% confidence), (2) LomuAI/Gemini 2.5 Flash (cheap, for platform failures), (3) I AM Architect/Claude Sonnet 4 (expensive, for agent failures). Features real-time monitoring, AgentFailureDetector for incident classification, multi-turn tool execution loop with read/write/search capabilities, and comprehensive session tracking for feedback loops.
