...
- **Task List System Fix (Nov 5, 2025)**: Fixed critical bug preventing LomuAI from creating task lists with progress bars. System prompt told LomuAI to call `createTaskList()` (camelCase) but actual tool name was `create_task_list()` (snake_case) → LomuAI silently failed when trying to create task lists → users lost all progress visibility ([commit 463990f](https://github.com/6-7Development/archetype/commit/463990f36efcd03de2cf6e01e8c2e7bc0234bab1)). Updated server/lomuSuperCore.ts to use correct snake_case tool names throughout. Task manager system with progress bars now works properly on both mobile and desktop.
