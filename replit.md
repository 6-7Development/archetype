...
- **Meta-SySop SSE Timeout Issue** (Oct 28, 2025): Meta-SySop sessions consistently terminate around iteration 13 (~90-120 seconds runtime), likely due to Railway's SSE connection duration limits. Despite 15-second heartbeat keepalives and MAX_ITERATIONS=25, sessions cut off before complex work completes. Root cause appears to be Railway load balancer hard timeout on SSE streams. Potential fixes: split work into smaller sessions with client-side reconnection, use webhook callbacks instead of long SSE streams, or implement chunked SSE with session resumption. Current workaround: Meta-SySop commits partial work to GitHub before timeout, triggering Railway auto-deployment of incomplete solutions.
