...
async function commitFile() {
  ...
  const { data: commit } = await octokit.git.createCommit({
    owner, repo,
    message: 'fix(railway): Check for server/public not dist/public\n\nThe Dockerfile copies frontend build to server/public, but the\nstartup script was checking for dist/public. This mismatch caused\nthe script to exit with "Frontend build is missing" error.\n\nFixed the check to match the actual location.',
    tree: tree.sha,
