/**
 * Gemini Context-Limit Protection
 * 
 * Prevents errors from exceeding Gemini's 1M token limit by:
 ...
 * 3. Maintaining 50K token safety margin (950K max)
 * 4. Iteratively dropping messages until under limit
...
// Gemini limits with safety margin
const GEMINI_MAX_TOKENS = 1000000; // 1M context for Gemini 2.5 Flash
const SAFETY_MARGIN = 50000; // 50K buffer for safety
const MAX_CONTEXT_TOKENS = GEMINI_MAX_TOKENS - SAFETY_MARGIN; // 950K

// Message preservation strategy
const MIN_RECENT_MESSAGES = 5;
const MAX_ATTACHMENT_SIZE = 100000; // Max chars per attachment (more generous than Claude)
...
function estimateMessageTokens(content: string | MessageContent[]): number {
  ...
  for (const block of content) {
    if (block.type === 'text' && block.text) {
      ...
      // Gemini images cost similar to Claude (~1270 tokens)
      totalTokens += 1270;
    ...
    if (block.type === 'text' && block.text && block.text.length > MAX_ATTACHMENT_SIZE) {
      ...
      if (lines.length > 400) {
        ...
        const truncatedLines = [
          ...lines.slice(0, 200),
          '\n... [TRUNCATED ' + (lines.length - 400) + ' lines] ...\n',
          ...lines.slice(-200)
        ];
        return {
          ...block,
...
/**
 * Create safe Gemini request with context-limit protection
 * 
 ...
 */
export function createSafeGeminiRequest(
  messages: GeminiMessage[],
  systemPrompt: string
): TruncationResult {
  ...
  console.log('[GEMINI-WRAPPER] Original context size:');
  ...
  console.log(`  - Limit: ${MAX_CONTEXT_TOKENS.toLocaleString()} tokens (${(SAFETY_MARGIN / 1000).toLocaleString()}K safety margin)`);
  
  // Step 2: Check if we're within limits
  if (originalTotalTokens <= MAX_CONTEXT_TOKENS) {
    console.log(`[GEMINI-WRAPPER] ✅ Within limits (${originalTotalTokens.toLocaleString()} / ${MAX_CONTEXT_TOKENS.toLocaleString()} tokens)`);
    console.log(`[GEMINI-WRAPPER] Safety margin: ${(MAX_CONTEXT_TOKENS - originalTotalTokens).toLocaleString()} tokens remaining`);
    return {
      messages,
      systemPrompt,
  ...
  console.log(`[GEMINI-WRAPPER] ⚠️ OVER LIMIT: ${originalTotalTokens.toLocaleString()} tokens exceeds ${MAX_CONTEXT_TOKENS.toLocaleString()}`);
  console.log(`[GEMINI-WRAPPER] Over by: ${(originalTotalTokens - MAX_CONTEXT_TOKENS).toLocaleString()} tokens`);
  console.log('[GEMINI-WRAPPER] Starting truncation process...');
  
  // Step 3: Truncate attachments first
  ...
  console.log(`[GEMINI-WRAPPER] After attachment truncation:`);
  ...
  if (tokensAfterAttachmentTruncation <= MAX_CONTEXT_TOKENS) {
    console.log('[GEMINI-WRAPPER] ✅ Attachment truncation successful');
    console.log(`[GEMINI-WRAPPER] Safety margin: ${(MAX_CONTEXT_TOKENS - tokensAfterAttachmentTruncation).toLocaleString()} tokens remaining`);
    return {
  ...
  console.log('[GEMINI-WRAPPER] Still over limit after attachment truncation');
  console.log('[GEMINI-WRAPPER] Iteratively removing oldest messages until under limit...');
  ...
  while (currentTotalTokens > MAX_CONTEXT_TOKENS && messagesToKeep > MIN_RECENT_MESSAGES) {
    ...
    console.log(`[GEMINI-WRAPPER] Trying ${messagesToKeep} messages: ${currentTotalTokens.toLocaleString()} tokens`);
  }
  
  // If still over limit, drop below MIN_RECENT_MESSAGES
  if (currentTotalTokens > MAX_CONTEXT_TOKENS) {
    console.log(`[GEMINI-WRAPPER] ⚠️ Still over limit with ${MIN_RECENT_MESSAGES} messages`);
    console.log('[GEMINI-WRAPPER] Dropping below minimum threshold...');
    ...
    while (currentTotalTokens > MAX_CONTEXT_TOKENS && messagesToKeep > 1) {
      ...
      console.log(`[GEMINI-WRAPPER] ⚠️ Below MIN threshold, trying ${messagesToKeep} messages: ${currentTotalTokens.toLocaleString()} tokens`);
    }
  }
  
  const removedCount = messagesWithTruncatedAttachments.length - messagesToKeep;
  
  console.log(`[GEMINI-WRAPPER] After message truncation:`);
  ...
  if (currentTotalTokens > MAX_CONTEXT_TOKENS) {
    console.error('[GEMINI-WRAPPER] ❌ CRITICAL ERROR: Cannot reduce context below limit');
    console.error(`[GEMINI-WRAPPER] Current: ${currentTotalTokens.toLocaleString()} tokens`);
    console.error(`[GEMINI-WRAPPER] Limit: ${MAX_CONTEXT_TOKENS.toLocaleString()} tokens`);
    console.error(`[GEMINI-WRAPPER] Over by: ${(currentTotalTokens - MAX_CONTEXT_TOKENS).toLocaleString()} tokens`);
  ...
  console.log(`[GEMINI-WRAPPER] ✅ Successfully reduced context to ${currentTotalTokens.toLocaleString()} tokens`);
  console.log(`[GEMINI-WRAPPER] Safety margin: ${(MAX_CONTEXT_TOKENS - currentTotalTokens).toLocaleString()} tokens remaining`);
 ...
 */
export function logGeminiTruncationResults(result: TruncationResult): void {
  if (!result.truncated) {
    console.log('[GEMINI-WRAPPER] ═══════════════════════════════════════');
    console.log('[GEMINI-WRAPPER] ✅ NO TRUNCATION NEEDED');
    console.log('[GEMINI-WRAPPER] ═══════════════════════════════════════');
    console.log(`[GEMINI-WRAPPER] Total tokens: ${result.estimatedTokens.toLocaleString()} / ${MAX_CONTEXT_TOKENS.toLocaleString()}`);
    console.log(`[GEMINI-WRAPPER] Safety margin: ${(MAX_CONTEXT_TOKENS - result.estimatedTokens).toLocaleString()} tokens`);
    return;
  }
  
  console.log('[GEMINI-WRAPPER] ═══════════════════════════════════════');
  console.log('[GEMINI-WRAPPER] ⚠️ CONTEXT TRUNCATED');
  console.log('[GEMINI-WRAPPER] ═══════════════════════════════════════');
  console.log(`[GEMINI-WRAPPER] Original tokens: ${result.originalTokens.toLocaleString()}`);
  console.log(`[GEMINI-WRAPPER] Final tokens: ${result.estimatedTokens.toLocaleString()}`);
  console.log(`[GEMINI-WRAPPER] Saved: ${(result.originalTokens - result.estimatedTokens).toLocaleString()} tokens (${Math.round((result.originalTokens - result.estimatedTokens) / result.originalTokens * 100)}%)`);
  console.log(`[GEMINI-WRAPPER] Messages removed: ${result.removedMessages}`);
  console.log(`[GEMINI-WRAPPER] Safety margin: ${(MAX_CONTEXT_TOKENS - result.estimatedTokens).toLocaleString()} tokens`);
  console.log('[GEMINI-WRAPPER] ═══════════════════════════════════════');
}
