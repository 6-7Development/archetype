...
export class PlatformHealingService {
  ...
  private readonly BACKUP_BRANCH_PREFIX = 'backup/beehive-';
  ...
  async createBackup(description: string): Promise<PlatformBackup> {
    try {
      ...
      try {
        ...
        const commitMessage = `[BeeHive Backup] ${description}`;
  ...
  async writePlatformFile(
    ...
    if (!quickValidation.valid) {
      ...
      throw new Error(
        ...
        `HINT: This prevents BeeHive from committing broken code with syntax errors.`
      );
    }
    ...
    // Prevents BeeHive from accidentally destroying files due to Gemini truncation bugs
    try {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      try {
        ...
        let commitMessage = `[BeeHive 🤖] Update ${filePath}`;
        ...
        if (!quickValidation.valid) {
          ...
          throw new Error(
            ...
            `This prevents BeeHive from pushing broken code to production.`
          );
        }
        ...
        try {
          ...
          if (!validation.valid) {
            ...
            throw new Error(
              ...
              `This prevents BeeHive from committing broken code with syntax errors.\n` +
          ...
          // Build enriched commit message with context
          const timestamp = new Date().toISOString().replace('T', ' ').split('.')[0];
          let commitMessage = `[BeeHive 🤖 DEV] Update ${filePath}`;
          if (context?.description) {
            commitMessage += `\n\n${context.description}`;
          }
          if (context?.sessionId || context?.taskId) {
  ...
  async createPlatformFile(
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      try {
        ...
        // Build enriched commit message with context
        const timestamp = new Date().toISOString().replace('T', ' ').split('.')[0];
        let commitMessage = `[BeeHive 🤖] Create ${filePath}`;
        if (context?.description) {
          commitMessage += `\n\n${context.description}`;
        }
        if (context?.sessionId || context?.taskId) {
    ...
    try {
      ...
      try {
        ...
        // Build enriched commit message with context
        const timestamp = new Date().toISOString().replace('T', ' ').split('.')[0];
        let commitMessage = `[BeeHive 🤖 DEV] Create ${filePath}`;
        if (context?.description) {
          commitMessage += `\n\n${context.description}`;
        }
        if (context?.sessionId || context?.taskId) {
  ...
  async commitChanges(message: string, changes: FileChange[]): Promise<string> {
    ...
    try {
      ...
      const commitMessage = `[BeeHive] ${message}\n\nChanges:\n${changes.map(c => `- ${c.operation} ${c.path}`).join('\n')}`;
  ...
  async deployAllChanges(userId: string): Promise<{ success: boolean; message: string; commitHash?: string; commitUrl?: string; filesDeployed: number }> {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        const githubService = getGitHubService();
        const commitMessage = `Batch deploy ${pendingChanges.length} file(s) via BeeHive\n\nFiles changed:\n${pendingChanges.map(c => `- ${c.operation} ${c.path}`).join('\n')}`;
