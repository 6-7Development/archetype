...
import { userCache, projectCache, subscriptionCache, responseCache } from './cache';
...
const BASE_SYSTEM_PROMPT = `🤖 WHO AM I?
...
function buildSystemPrompt(mode: string, existingFiles: any[] = [], chatHistory: any[] = [], userSecrets: Record<string, string> = {}): string {
...
I wrap async operations in try/catch, show user-friendly error messages, use structured logging, and implement retry with backoff. I add data-testid attributes to interactive elements, write unit tests for logic, and integration tests for APIs. Webhooks and background jobs use unique IDs to prevent duplicate processing (idempotency). And for databases, I index foreign keys and queried columns, and use transactions for multi-step operations.`;
  ...
  // Rest of the static content continues in the request handler where files/history/secrets are added dynamically
...
function cacheResponse(ttlSeconds: number = 5) {
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  wss.on('connection', (ws: any) => {
    ...
    // Initialize session tracking and keep-alive status
    ...
    // Handle WebSocket errors to prevent memory leaks
...
Keep it under 150 words. Be factual and specific.`,
  ...
  app.get('/api/auth/user', isAuthenticated, cacheResponse(5), async (req: any, res) => {
    try {
      const userId = req.authenticatedUserId;
      const user = await storage.getUser(userId);
      res.json(user);
