...
import { analyzePerformance } from '../tools/performance-analyzer.ts';
  ...
  try {
    ...
    const toolExecutor = async (signal: AbortSignal) => {
      try {
        ...
        // Proactive Code Intelligence Tools (Gap #5, #7, #8, #9)
        ...
        } else if (toolName === 'analyze_performance') {
          return await analyzePerformance(input);
        }
...
export function registerChatRoutes(app: Express, dependencies: { wss: any }) {
  ...
  app.get("/api/platform-health", async (req: any, res) => {
    try {
      ...
      // Get active BeeHiveAI jobs (placeholder - would come from job manager)
      const activeJobs = 0;
  ...
  // GET /api/beehive-ai/jobs - List BeeHiveAI jobs
  app.get("/api/beehive-ai/jobs", async (req: any, res) => {
    try {
