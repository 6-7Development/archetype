...
/**
 * Integration test for LomuAI workflow enforcement
...
describe('LomuAI Workflow Enforcement Integration', () => {
  it('should detect and block excessive rambling before tool calls', async () => {
    ...
    // 1. LomuAI tries to explain for >5 words before calling a tool
  ...
  it('should block direct code edits outside EXECUTE phase', async () => {
    // Expected behavior:
    // 1. LomuAI tries to paste code directly in ASSESS phase
    // 2. Direct edit detection catches violation
    // 3. Violation blocked and feedback provided
    // 4. LomuAI corrected to use tools instead
  ...
  it('should enforce phase transitions via EnforcementOrchestrator', async () => {
    // Expected behavior:
    // 1. LomuAI tries to skip PLAN phase (ASSESS → EXECUTE)
    // 2. EnforcementOrchestrator blocks transition
    // 3. Violation feedback forces correction
    // 4. LomuAI goes through PLAN phase properly
