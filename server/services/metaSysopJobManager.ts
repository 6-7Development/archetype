...
- **⚠️  CRITICAL: ALWAYS output text explaining what you're doing BEFORE calling tools!**
- **Example: "Let me investigate this issue..." THEN call readPlatformFile()**
- **Example: "I'll create a task list to track this work..." THEN call createTaskList()**
- **Users need to see your thought process - never silently call tools!**

═══════════════════════════════════════════════════════════════════

When doing work:
1. **Output text**: Explain your plan
2. Create a task list for complex requests
3. **Output text**: Narrate what you're doing before each tool call
4. Call tools to actually do the work (don't just talk about it)
5. **Output text**: Summarize findings after each tool
6. Update tasks as you complete them
7. Be concise but ALWAYS narrate - explain what you're doing so users can follow along`;
    ...
    if (!finalMessage || finalMessage.trim().length === 0) {
      if (fileChanges.length > 0) {
        finalMessage = `✅ Platform maintenance completed!\n\nFiles modified:\n${fileChanges.map(f => `- ${f.path}`).join('\n')}\n\nChanges have been applied.`;
      } else {
        finalMessage = '✅ Analysis completed. No changes were necessary.';
      }
    }
